...
def post_process(x,stride = torch.tensor([8,16,32],dtype=torch.float32)):
    ...
    # x_cat = torch.cat([xi.view(1, 5, -1) for xi in x], 2)
    # x_cat = torch.cat([xi.view(shape0, shape1, -1) for xi in x], 2)
    box, cls = x.split((4, 1), 1)
    anchors, strides = (x.transpose(0, 1) for x in make_anchors(x, stride, 0.5))
    dbox = dist2bbox(box, anchors.unsqueeze(0), xywh=False, dim=1) * strides
    y = torch.cat((dbox, cls.sigmoid()), 1)

    return y
def post_process2(x,stride = torch.tensor([8,16,32],dtype=torch.float32)):
    
    box, cls = x
    # cls = cls
    y = torch.cat((box, cls), 2)
    return y
def make_anchors(feats, strides, grid_cell_offset=0.5):
    """Generate anchors from features."""
    anchor_points, stride_tensor = [], []
    assert feats is not None
    feats = [[40,24],[20,12],[10,6]]
...
def non_max_suppression(
        ...
        conf_thres=0.5,
        iou_thres=0.45,
        nc=2,  # number of classes (optional)
        max_wh=7680,
):
...
def plot_boxs(preds,save_path):
    img0 = cv2.imread(img_path)
    for pred in preds:
        box = [int(a) for a in pred[:4]]
        conf = str(float(pred[-2]))[:4]    
        cls = int(pred[-1])
        cv2.rectangle(img0, (box[0], box[1]),(box[2], box[3]), (56, 56, 255), 3,lineType=cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    # onnx 模型加载
    # onnx_path = fr"\\192.168.1.8\algorithm\lijiaqi\qidian\weights\{task}.onnx"

    onnx_path = r"/mnt/pai-storage-8/tianyuan/pfd/sqyolov8/weights/base/8n2_640_0107/weights/best.onnx"
    # onnx_path = r"/mnt/pai-storage-1/tianyuan/workspace/smoke/sqyolov8/runs/detect/smoke_det0809_8/weights/best.onnx"
    onnx_test = ONNXModel(onnx_path)

 
    img_path = '/mnt/pai-storage-8/tianyuan/face_qrcode_det/yolov8/test.png'
    # save_path = '/mnt/pai-storage-1/tianyuan/workspace/facedet/sqyolov8/test_imgs/test_data/pred_1411.png'
    orig_img = cv2.imread(img_path)

    # 前处理
    im = pre_process(img_path,[320,192])
    ...
    # 后处理
    onnx_result = [torch.tensor(xi) for xi in onnx_result]
    onnx_result = torch.cat(onnx_result,1)
    preds = post_process(onnx_result)

    # NMS
    pred = non_max_suppression(preds,nc=1)[0]
    ...
    if pred.shape[0] != 0 :
        ...
        for pr in pred:
            # print(pr)
            box = [int(a) for a in pr[:4]]
            conf = str(float(pr[4])) 
            cls = int(pr[5])
            cv2.rectangle(orig_img, (box[0], box[1]),(box[2], box[3]), (56, 56, 255), 3,lineType=cv2.LINE_AA)
            cv2.putText(orig_img, str(cls) + ' ' + str(conf), (box[0], box[1]-5), 0, 0.6, (56, 56, 255), 1, lineType=cv2.LINE_AA)
        cv2.imwrite('./haitun.png',orig_img)
    print('检测完成')
